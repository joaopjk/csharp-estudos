...
public class ImageService(OpenAIClient openAiClient)
{
    private readonly string _model = Environment.GetEnvironmentVariable("IMAGE_MODEL") ?? "dall-e-3";
    ...
    public async Task<IEnumerable<string>> GenerateImage(
        ...
        int width = 1024)
    {
        var imageClient = openAiClient.GetImageClient(_model);
        ...
        var response = await imageClient.GenerateImagesAsync(
            prompt,
            numberOfImages,
            options);
